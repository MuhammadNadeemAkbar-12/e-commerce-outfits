<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 text-slate-100 px-4 py-10">
    <div class="mx-auto w-full max-w-3xl">
      <div class="rounded-3xl border border-slate-800 bg-slate-900/85 backdrop-blur-xl shadow-2xl overflow-hidden">
        <div class="px-6 sm:px-10 pt-8 pb-6 border-b border-slate-800">
          <h1 class="text-2xl sm:text-3xl font-bold tracking-tight">Admin Password Recovery</h1>
          <p class="text-slate-400 mt-2 text-sm sm:text-base">
            Reset admin credentials securely using email and reset token.
          </p>
        </div>
        ...
        <div class="p-6 sm:p-10 grid gap-8 lg:grid-cols-2">
          <section class="space-y-4">
            ...
            <label class="text-sm text-slate-300 block">Admin Email</label>
            <input
              v-model.trim="email"
              type="email"
            ...
            >
              <span v-if="!sending">Send Admin Reset Link</span>
          ...
          <section class="space-y-4">
            ...
            >
              <span v-if="!resetting">Reset Admin Password</span>
...
const sendResetLink = async () => {
  ...
  if (!/\S+@\S+\.\S+/.test(email.value)) {
    errorMessage.value = 'Please enter a valid admin email address.'
    return
  }
  ...
  try {
    ...
    if (res.success) linkMessage.value = res.message || 'Admin reset link sent successfully.'
